...
Escribe la fecha actual (**NOTE-01**): 05/06/2021
...
```{r}
NOMBRE_COMPLETO = "Francisco Javier Zavala Torres"
DIGITOS_MATRICULA = 01746851
```
...
- Cantidad de transacciones (n) (**NOTE-02**): 
- Frecuencia absoluta del item {beer} (**NOTE-03**): 
- Frecuencia absoluta del item {nuts} (**NOTE-04**): 
- Frecuencia absoluta del itemset {beer, nuts} (**NOTE-05**): 
- Support del item {beer} (**NOTE-06**): 
- Support del itemset {beer, nuts} (**NOTE-07**): 
...
- Número de renglones (**NOTE-08**): 5 renglones
- Número de columnas (**NOTE-09**): 6 columnas
...
- ¿Cuál es la interpretación de la regla `{} => {diapers}` en la que se aprecia la ausencia del LHS? (**NOTE-19**):
Existe un lift de 1 indicando que x y y son independientes por lo tanto la regla no representa un patrón real y por eso no existe un item en lhs.  

- Calcula el valor de la métrica support para la regla `{beer} => {nuts}` (**NOTE-20**): 0.2

- Calcula el valor de la métrica confidence para la regla `{beer} => {nuts}` (**NOTE-21**): 1/3 

- ¿Por qué no aparece listada la regla `{beer} => {nuts}`? (**NOTE-22**): Porque al momento de correr el algoritmo apriori, su confidence es menor a 0.8 y esto hace que no se muestre ya que la confidence es 0.3333
...
- ¿Cuál es el propósito de `session id`? (**NOTE-23**): Es la primary key de las sessions, su proposito es identificar las sesiones

- ¿Cuál es el propósito de `track id`? (**NOTE-24**): Es la primary key de las tracks, su proposito es identificar las tracks

- ¿Consideras que las otras columnas del dataset son aplicables para análisis por medio de reglas de asociación, justifica tu respuesta? (**NOTE-25**): Si, ya que algunas otras columnas pueden dar un enfoque distinto al analisis, como por ejemplo el de hour of day que puede mostrar a que horas estas mas activo.
...
- ¿Cuál es el ID y frecuencia absoluta de la sesión más frecuente? (**NOTE-26**): ID = t_bacf06d3-9185-4183-84ea-ff0db51475ce, frecuencia = 865

- ¿Cuántos items tiene el itemset con mayor número de ocurrencias en la lista element length distribution? (**NOTE-27**): 1591

- En el contexto de este dataset con información de sesiones de streaming, ¿cuál es la interpretación de la media aritmética? (**NOTE-28**): Las sesiones promedio de un usuario son 14.61, esto quiere decir que pasaron esuchando musica interrumpida por 1 min 14.61 veces, aproximadamente 15
...
- ¿Qué factores están asociados con que el algoritmo `apriori` con parámetros por omisión no haya producido ninguna regla de asociación, justifica tu respuesta? (**NOTE-29**): El alogritmo apriori solo muestra reglas encima del minimo de confidence y support, si no mostro ninguna regla, es que todos los valores se encuentran debajo del minimo

- ¿Qué riesgos identificas con la asignación de valores muy bajos en los parámetros de  `support` y `confidence` al invocar el algoritmo `apriori`? Valores de support y confidence bajos son malos y si comenzamos a mostrarlos, se analizaron reglas no relevantes complicando el analisis. 
